...
function aesthetics(data){
	...
	//to do: implement
	//to do: more robust handling of negative values
	...
	var aes_radius = function(variable){
		...
		var domain = [0, distribution.absmax];

		var scale = d3.scaleSqrt().domain(domain).range([minR, maxR]);
		...
		aes.radii = function(min_radii, max_radii, missing_r){
			...
			scale = d3.scaleSqrt().domain(domain).range([minR, maxR]);
		...
		//set scale breaks manually - breaks.length should equal sizes.length-1;
		aes.breaks = function(breaks, sizes){
			scale = function(v){
				var i = -1;
				var r = null;
		...
		aes.domain = function(custom_domain){
			if(arguments.length){
				domain = custom_domain;
				scale.domain(custom_domain);
				set_scale();
				return aes;
			}
			else{
		...
		aes.ticks = function(values){
			var vals;
			if(arguments.length){
				vals = values;
			}
			else{
				vals = d3.range(0, 9, 4).map(function(i){return distribution.deciles[i]});
			}
...
//To do: selectively call tooltip on enter only? Or just add this to api

//called with dom==tooltip_dom from main.js
function tooltips(layer, dom, annotation_group){

	var tip = dom.tip;
	var tip_inner = dom.tip_inner;
	var tip_test = dom.test; //area to test tooltip content width

	var parent_node = tip.node().parentNode;

	var min_tip_width = 25; 
	var tip_width = null; //fixed tip width

	var tip_pad = 20; //see tip_pad from main.js
	...
	//hide tooltips
	function hide(){
		tip.style("visibility","hidden").style("left", "0%").style("top","0%").style("z-index","1");
		dom.arrows.L.style("visibility","hidden");
		dom.arrows.R.style("visibility","hidden");
	}

	//determine the longest piece of content in tooltip test area
	function longest_content(){
		var box = tip_test.node().getBoundingClientRect();
		var w = Math.round(box.right - box.left) + (tip_pad*2) + 1;
		
		//if fixed tip_width is set, use that, otherwise 
		var final = tip_width != null ? tip_width : 
										(w > min_tip_width ? w : min_tip_width);

		//lastly, don't exceed container viewport width
		return (final <= container_viewport.width*0.9) ? final : (container_viewport.width*0.9);
	}
	...
	function show(pos){
			...
			//locally scoped width
			var tip_width = longest_content();
		...
		tip.style("visibility","visible")
		   ...
		   .style("width",(tip_width+(2*tip_pad))+"px")
		   ;
	}
	...
	T.apply = function(sel, build){
		...
		sel.on("mouseenter", function(d,i){
			...
			build.call(tip_test.node(), d); //render the tooltip content to a testing area
			show_mouse(d,i);
		});
...
//to do: shouldn't attr and style methods just operate directly on svg selection
...
//to do: document all the data/methods of a layer. simplify the scales. make sure if you bind new data there aren't side effects (new data should wipe out scales/aesthetics or recalculate them)
		//to do: once all the pieces and the process of drawing a map is mapped out, refactor code to make more robust and modular
...
function layer(){
	...
	//var aes = {}; //private repo of aesthetics
	//these should have default constants // or a lookup of defaults
	//e.g. aes.stroke = "#dddddd"; aes.fill = "none"; etc.
	//create methods for L.fill(), L.stroke(), L.r(), etc. Setting null or not setting will apply defaults
	//can pass fns or constants or null (to reset) -- what should these functions look like? they should be scales
	//  these can be scales that are manually created, but there should be some kind of convenience methods for generating scales
	//add L.selection() to return the geo selection (d3) primitive -- circles or paths// get rid of the ability to add on geos to a layer as it just overly complicates things
	...
	L.tooltips = function(fn, dfn, width){
...
function legend(container){
	...
	L.swatch = function(container, values, formatter, title){
		
		var format = typeof formatter == "function" ? formatter : function(v){return v};

		var wrap = d3.select(container);

		var swatch_legend_u = wrap.selectAll("div.legend-swatch").data([values]);
		swatch_legend_u.exit().remove();
		var swatch_legend = swatch_legend_u.enter().append("div").classed("c-fix legend-swatch", true)
						.merge(swatch_legend_u);

		var tiles_u = swatch_legend.selectAll("div.legend-tile").data(values);

		tiles_u.exit().remove();
		var tiles_e = tiles_u.enter().append("div")
							.classed("legend-tile c-fix", true)
							.style("float","left")
							.style("margin","0.25rem 1rem 0.25rem 0rem")
							.style("line-height","1rem");

		tiles_e.append("div").classed("legend-tile-swatch", true)
				.style("width","0.75rem").style("height","1rem").style("display","inline-block")
				.style("margin","0rem 5px 0rem 0px")
				.style("vertical-align","top")
				;

		tiles_e.append("p").classed("legend-tile-label", true)
		...
		var tiles = tiles_e.merge(tiles_u);

		tiles.select("div.legend-tile-swatch").style("background-color", function(d){return d.color});

		tiles.select("p.legend-tile-label").text(function(d,i){return format(d.value);});

		if(title != null){
			var titles = swatch_legend.selectAll("p.legend-title").data([title]);
			titles.exit().remove();
			titles.enter().append("p").classed("legend-title",true)
				  .merge(titles).html(function(d){return d})
				  .style("clear","both")
...
//to do: zoom out is off a little bit -- see digitalization map
...
function mapd(root_container){	
	...
	//div to render test content to dynamically determine width of tooltip
	var tip_test = container_wrap.append("div")
								 .style("width","100%")
								 .style("height","100%")
	...
	var map_canvas = outer_wrap.append("canvas").style("width","100%").style("height","100%").style("position","absolute").style("z-index","2");
	var map_svg = outer_wrap.append("svg").attr("width","100%").attr("height","100%").style("position","relative").style("z-index","3");

	//TOOLTIP
	var tip_pad = 20;
	var tooltip_wrap = container_wrap.append("div")
								 .style("position","absolute")
								 .style("visibility","hidden")
								 .style("opacity","0")
	...
	var zoom_button = outer_wrap.append("div").style("position","absolute").style("top","60%").style("right","10px")
	...
	var zoom_in_g = zoom_svg.append("g").attr("transform","translate(0,-1050)")
											.attr("stroke","#444444")//.attr("stroke","#0d73d6")
	...
	map.zoomLevels = function(num_levels){
		if(num_levels > 0 && num_levels <= zoom_scale.length){
			zoom_scale = zoom_scale.slice(0, num_levels);
		}
